package com.model;

public class YoAdvance {
    private String approveNo;

    private String title;

    private String approveStatus;

    private String approveResult;

    private String askStartTime;

    private String askEndTime;

    private String askStaffId;

    private String askStaffName;

    private String askStaffDep;

    private String approverHistory;

    private String approveRecord;

    private String approverNow;

    private String approveCost;

    private String advanceStartTime;

    private String advanceAimMc;

    private String advanceAimSc;

    private String advanceSum;

    private String advancePayee;

    private String advanceBank;

    private String advanceBranch;

    private String advanceBankAccount;

    private String advanceReason;

    private String other;

    private String exportStatus;

    public String getApproveNo() {
        return approveNo;
    }

    public void setApproveNo(String approveNo) {
        this.approveNo = approveNo;
    }

    public String getTitle() {
        return title;
    }

    public void setTitle(String title) {
        this.title = title;
    }

    public String getApproveStatus() {
        return approveStatus;
    }

    public void setApproveStatus(String approveStatus) {
        this.approveStatus = approveStatus;
    }

    public String getApproveResult() {
        return approveResult;
    }

    public void setApproveResult(String approveResult) {
        this.approveResult = approveResult;
    }

    public String getAskStartTime() {
        return askStartTime;
    }

    public void setAskStartTime(String askStartTime) {
        this.askStartTime = askStartTime;
    }

    public String getAskEndTime() {
        return askEndTime;
    }

    public void setAskEndTime(String askEndTime) {
        this.askEndTime = askEndTime;
    }

    public String getAskStaffId() {
        return askStaffId;
    }

    public void setAskStaffId(String askStaffId) {
        this.askStaffId = askStaffId;
    }

    public String getAskStaffName() {
        return askStaffName;
    }

    public void setAskStaffName(String askStaffName) {
        this.askStaffName = askStaffName;
    }

    public String getAskStaffDep() {
        return askStaffDep;
    }

    public void setAskStaffDep(String askStaffDep) {
        this.askStaffDep = askStaffDep;
    }

    public String getApproverHistory() {
        return approverHistory;
    }

    public void setApproverHistory(String approverHistory) {
        this.approverHistory = approverHistory;
    }

    public String getApproveRecord() {
        return approveRecord;
    }

    public void setApproveRecord(String approveRecord) {
        this.approveRecord = approveRecord;
    }

    public String getApproverNow() {
        return approverNow;
    }

    public void setApproverNow(String approverNow) {
        this.approverNow = approverNow;
    }

    public String getApproveCost() {
        return approveCost;
    }

    public void setApproveCost(String approveCost) {
        this.approveCost = approveCost;
    }

    public String getAdvanceStartTime() {
        return advanceStartTime;
    }

    public void setAdvanceStartTime(String advanceStartTime) {
        this.advanceStartTime = advanceStartTime;
    }

    public String getAdvanceAimMc() {
        return advanceAimMc;
    }

    public void setAdvanceAimMc(String advanceAimMc) {
        this.advanceAimMc = advanceAimMc;
    }

    public String getAdvanceAimSc() {
        return advanceAimSc;
    }

    public void setAdvanceAimSc(String advanceAimSc) {
        this.advanceAimSc = advanceAimSc;
    }

    public String getAdvanceSum() {
        return advanceSum;
    }

    public void setAdvanceSum(String advanceSum) {
        this.advanceSum = advanceSum;
    }

    public String getAdvancePayee() {
        return advancePayee;
    }

    public void setAdvancePayee(String advancePayee) {
        this.advancePayee = advancePayee;
    }

    public String getAdvanceBank() {
        return advanceBank;
    }

    public void setAdvanceBank(String advanceBank) {
        this.advanceBank = advanceBank;
    }

    public String getAdvanceBranch() {
        return advanceBranch;
    }

    public void setAdvanceBranch(String advanceBranch) {
        this.advanceBranch = advanceBranch;
    }

    public String getAdvanceBankAccount() {
        return advanceBankAccount;
    }

    public void setAdvanceBankAccount(String advanceBankAccount) {
        this.advanceBankAccount = advanceBankAccount;
    }

    public String getAdvanceReason() {
        return advanceReason;
    }

    public void setAdvanceReason(String advanceReason) {
        this.advanceReason = advanceReason;
    }

    public String getOther() {
        return other;
    }

    public void setOther(String other) {
        this.other = other;
    }

    public String getExportStatus() {
        return exportStatus;
    }

    public void setExportStatus(String exportStatus) {
        this.exportStatus = exportStatus;
    }

	@Override
	public String toString() {
		return "YoAdvance [approveNo=" + approveNo + ", title=" + title
				+ ", approveStatus=" + approveStatus + ", approveResult="
				+ approveResult + ", askStartTime=" + askStartTime
				+ ", askEndTime=" + askEndTime + ", askStaffId=" + askStaffId
				+ ", askStaffName=" + askStaffName + ", askStaffDep="
				+ askStaffDep + ", approverHistory=" + approverHistory
				+ ", approveRecord=" + approveRecord + ", approverNow="
				+ approverNow + ", approveCost=" + approveCost
				+ ", advanceStartTime=" + advanceStartTime + ", advanceAimMc="
				+ advanceAimMc + ", advanceAimSc=" + advanceAimSc
				+ ", advanceSum=" + advanceSum + ", advancePayee="
				+ advancePayee + ", advanceBank=" + advanceBank
				+ ", advanceBranch=" + advanceBranch + ", advanceBankAccount="
				+ advanceBankAccount + ", advanceReason=" + advanceReason
				+ ", other=" + other + ", exportStatus=" + exportStatus + "]";
	}
    
}